...
void Test_HikOnline()
{
    ...
    memcpy(struCountryIDCond.szClientVersion, "5.2.5.2 build20160715", strlen("5.2.5.2 build20160715"));
...
//    BOOL suc = NET_DVR_UpdateRecordIndex(iUserID,iChan);
    
    NET_DVR_FILECOND pFindCond = {0};
    pFindCond.struStartTime.dwYear = 2016;
    pFindCond.struStartTime.dwMonth = 12;
    pFindCond.struStartTime.dwDay = 18;
    
    pFindCond.struStopTime.dwYear = 2016;
    pFindCond.struStopTime.dwMonth = 12;
    pFindCond.struStopTime.dwDay = 24;
    pFindCond.lChannel = iChan;
    pFindCond.dwFileType = 0xff;
    pFindCond.dwIsLocked = 0xff;
    pFindCond.dwUseCardNo = 0;
    
    LONG hFindHandle = NET_DVR_FindFile_V30(iUserID,&pFindCond);
    if(-1 == hFindHandle)
    {
        printf("查找失败");
        return;
    ...
    else
    {
        ...
        while(ret>0)
        {
            ...
            {
                //保存文件信息
                printf("\n FindData.sFileName : %s",FindData.sFileName);
                printf("\n FindData.startTime : %d:%d:%d",FindData.struStartTime.dwYear,FindData.struStartTime.dwMonth,FindData.struStartTime.dwDay);
